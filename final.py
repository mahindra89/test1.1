...
if not st.session_state.page_table_initialized:
    ...
    with col1:
        st.markdown("### Virtual Memory Size")
        st.markdown(f"## {simulator.virtual_size} KB")

        st.markdown("### Virtual Address Bits")
        st.markdown(f"## {simulator.total_bits}")

        st.markdown("### Page Index Bits")
        st.markdown(f"## {simulator.index_bits}")

        st.markdown("### Number of Virtual Pages")
        st.markdown(f"## {simulator.num_virtual_pages}")

        st.markdown("### First Virtual Address")
        st.markdown(f"## 0x0")

        st.markdown("### Last Virtual Address")
        st.markdown(f"## 0x{virtual_max_address:X}")

    with col2:
        st.markdown("### Physical Memory Size")
        st.markdown(f"## {simulator.physical_size} KB")

        st.markdown("### Physical Address Bits")
        st.markdown(f"## {simulator.physical_address_bits}")

        st.markdown("### Physical Frame Bits")
        st.markdown(f"## {simulator.physical_frame_bits}")

        st.markdown("### Number of Physical Frames")
        st.markdown(f"## {simulator.num_physical_frames}")

        st.markdown("### First Physical Address")
        st.markdown(f"## 0x0")

        st.markdown("### Last Physical Address")
        st.markdown(f"## 0x{physical_max_address:X}")

    with col3:
        st.markdown("### Page Size")
        st.markdown(f"## {simulator.page_size} KB")

        st.markdown("### Offset Bits")
        st.markdown(f"## {simulator.offset_bits}")

        st.markdown("### Replacement Algorithm")
        st.markdown("## FIFO")

        st.markdown("### Conversion Type")
        st.markdown(f"## {'Virtual to Physical' if simulator.conversion_type == 'virtual_to_phys' else 'Physical to Virtual'}")
    ...
    with st.expander("Configure Page Table Entries and Arrival Order"):
        ...
        # Display frame capacity information
        st.info(f"Physical memory has {max_allowed_pages} frames available. You can mark at most {max_allowed_pages} virtual pages as present.")
